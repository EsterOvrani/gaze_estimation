...
VisionRunningMode = mp.tasks.vision.RunningMode
    ...
    #"C:\\Users\\Esti\\PycharmProjects\\Gaze_estimation-master\\face_landmarker.task"),
options = FaceLandmarkerOptions(
    base_options=BaseOptions(model_asset_path='/Users/shai/projects/gaze_estimation/Gaze_estimation-master/face_landmarker.task'), 
...
def simulation(t_vec, r_vec):
    ...
    plane1 = origin + s[:, :, np.newaxis] * vector1 + t[:, :, np.newaxis] * vector2
    ...
    origin_rotated = rotation.apply(origin - t_vec.flatten())  #TODO: to check if it's fine to mines the t_vec instead of to add t_vec.

    # image plane
    plane2 = origin_rotated + s[:, :, np.newaxis] * v1_rotated + t[:, :, np.newaxis] * v2_rotated
    ...
    ax.scatter(origin_rotated[0], origin_rotated[1], origin_rotated[2], color='green', s=1)

    #find the cut point
    # o' + x1*v1 +x2*v2 = x3*d
    a = np.vstack([v1_rotated, v2_rotated, -face_diraction]).T  
    p = np.linalg.solve(a, -origin_rotated)


    ax.plot([origin_rotated[0], origin_rotated[0]+p[0]*v1_rotated[0] +\
                                p[1]*v2_rotated[0]], 
            [origin_rotated[1], origin_rotated[1]+p[0]*v1_rotated[1] +\
                                p[1]*v2_rotated[1]], 
            [origin_rotated[2], origin_rotated[2]+p[0]*v1_rotated[2] +\
                                p[1]*v2_rotated[2]], 
            label='Line between points', marker='o')
    ax.plot([origin[0], origin[0]+p[2]*face_diraction[0]], 
            [origin[1], origin[1]+p[2]*face_diraction[1]], 
            [origin[2], origin[2]+p[2]*face_diraction[2]], 
            label='Line between points', marker='o')
